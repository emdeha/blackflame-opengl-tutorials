...
#ifdef HOMEWORK
glutil::MatrixStack g_persMatrix;
glutil::MatrixStack g_camMatrix;
#endif
...
void display()
{
	...
	if(g_pConeMesh && g_pCylinderMesh && g_pCubeTintMesh && g_pCubeColorMesh && g_pPlaneMesh)
	{
		...
		g_camMatrix.SetMatrix(CalcLookAtMatrix(camPos, g_camTarget, glm::vec3(0.0f, 1.0f, 0.0f)));
#elif defined HOMEWORK
		const glm::vec3 &camPos = ResolveCamPosition();
		g_camMatrix.SetMatrix(CalcLookAtMatrix(camPos, g_camTarget, glm::vec3(0.0f, 1.0f, 0.0f)));

		glutil::MatrixStack worldMatrix;
		worldMatrix.SetMatrix(g_persMatrix.Top() * g_camMatrix.Top());

		glUseProgram(UniformColor.theProgram);
		glUniformMatrix4fv(UniformColor.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(worldMatrix.Top()));
		glUseProgram(ObjectColor.theProgram);
		glUniformMatrix4fv(ObjectColor.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(worldMatrix.Top()));
		glUseProgram(UniformColorTint.theProgram);
		glUniformMatrix4fv(UniformColorTint.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(worldMatrix.Top()));
		glUseProgram(0);
#else
		const glm::vec3 &camPos = ResolveCamPosition();
...
#elif defined HOMEWORK
			glUniformMatrix4fv(ObjectColor.modelToWorldMatrixUnif, 1, GL_FALSE, glm::value_ptr(modelMatrix.Top()));
			glUniformMatrix4fv(ObjectColor.worldToClipMatrixUnif, 1, GL_FALSE, glm::value_ptr(g_persMatrix.Top()));
#else			
			glm::mat4 identity(1.0f);
			glUniformMatrix4fv(ObjectColor.modelToWorldMatrixUnif, 1, GL_FALSE, glm::value_ptr(modelMatrix.Top()));
			glUniformMatrix4fv(ObjectColor.worldToCameraMatrixUnif, 1, GL_FALSE, glm::value_ptr(identity));
#endif
			g_pCubeColorMesh->Render();
			glUseProgram(0);
			glEnable(GL_DEPTH_TEST);
...
void reshape (int w, int h)
{
	...
	glUseProgram(0);
#elif defined HOMEWORK
	g_persMatrix.Perspective(45.0f, (w / (float)h), g_fzNear, g_fzFar);

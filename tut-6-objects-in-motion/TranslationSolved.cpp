...
// Ref:
glm::mat4 StationaryOffsetMat(float fElapsedTime)
{
	glm::mat4 theMat(1.0f);

	theMat[3] = glm::vec4(0.0f, 0.0f, -20.0f, 1.0f);

	return theMat;
}

glm::mat4 OvalOffsetMat(float fElapsedTime)
{
	const float fLoopDuration = 3.0f;
	const float fScale = 3.14159f * 2.0f / fLoopDuration;
	...
	glm::mat4 theMatScale(1.0f);

	theMatTrans[3] = glm::vec4(0.0f, 5.0f, -20.0f, 1.0f);
	theMatScale[1].y = 0.5f;
	 
	theMatRot[0].x = cosf(fCurrTimeThroughLoop * fScale);
	theMatRot[0].y = sinf(fCurrTimeThroughLoop * fScale) * 2.0f;
	theMatRot[1].x = -sinf(fCurrTimeThroughLoop * fScale);
	theMatRot[1].y = cosf(fCurrTimeThroughLoop * fScale) * 2.0f;


	theMatRotOrient[0].x = cosf(-fCurrTimeThroughLoop * fScale);
	theMatRotOrient[0].y = sinf(-fCurrTimeThroughLoop * fScale);
	theMatRotOrient[1].x = -sinf(-fCurrTimeThroughLoop * fScale);
	theMatRotOrient[1].y = cosf(-fCurrTimeThroughLoop * fScale);
...
struct Instance
{
	...
	typedef glm::mat4(*OffsetFuncMat)(float); //Ref:

	//OffsetFunc CalcOffset; 
	OffsetFuncMat CalcOffsetMat; //Ref:
	...
	glm::mat4 ConstructMatrix(float fElapsedTime)
	{
		...
		theMat = CalcOffsetMat(fElapsedTime);
...
// Ref:
Instance g_instanceListMat[] =
{
	{StationaryOffsetMat},
	{OvalOffsetMat},
	{BottomCircleOffsetMat},
};
//*****
...
void Display()
{
	...
	for(int iLoop = 0; iLoop < ARRAY_COUNT(g_instanceListMat); iLoop++)
	{
		Instance &currInst = g_instanceListMat[iLoop];
